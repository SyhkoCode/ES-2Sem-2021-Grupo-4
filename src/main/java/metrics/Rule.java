...
public class Rule {
	 ...
	 * 
	 * @param nome       The name of the code smell that can be detected with this
	 *                   rule example: "is_God_Class"
	 * @param expression String that represents the actual rule example: "SE (
	 *                   LOC_class > 5 OU LOC_class > 100 )"
	 ...
	 * <p>
	 * Using the data extracted from the code of a method, it sees if the method
	 * tests positive for the code smell represented by the rule.
	 * <p>
	 * Copies and changes the expression of the rule so it resembles a boolean
	 * expression. Then changes the names of the metrics the rule checks with the
	 * corresponding data from the methodData. The final boolean result is
	 * calculated by a JavaScript engine.
	 * 
	 ...
	 * @throws IllegalStatementException if the final expression can't be turned
	 *                                   into a boolean by the JavaScript engine
	 * @see MethodData
	 */
	 ...
	 * <p>
	 * Using a scanner it reads a file containing the lines necessary to create
	 * rules. Each rule is 2 lines, the 1st is the code smell it detects, the 2nd
